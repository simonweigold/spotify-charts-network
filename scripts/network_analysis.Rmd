...
global %>% 
  ...
  geom_freqpoly(bins = length(streams), col = "coral")
```
Most occuring artists and wordcloud
```{r}
dplyr::count(df, artist, sort = T) %>% head(25)

wordcloud2::wordcloud2(data = count(df, artist), size = 1, color = "random-dark")
```
...
wordcloud2::wordcloud2(data = count(df, title), size = 1, color = "random-dark")
```
